...
typename List<T>::ListNode* List<T>::split(ListNode* start, int splitPoint) {
    ...
    // find node
    if(start==nullptr||splitPoint<1) return start;
...
typename List<T>::ListNode* List<T>::merge(ListNode* first, ListNode* second) {
  ...
  while(1){
  // if any one of the list is empty, just connect and quit
    if(first==nullptr){
      head->next=second;
      second->prev=head;
      break;
  ...
  // chose min in lists and step forward
    if(second->data<first->data){
      head->next=second;
      second->prev=head;
      second=second->next;
