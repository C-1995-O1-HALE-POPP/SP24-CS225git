...
class KDTree
{
  ...
  private:
    ...
    /* helper function to recursively make a valid tree */
    KDTree<Dim>::KDTreeNode* maketree(vector<Point<Dim>>& points, int leftIdx, int rightIdx, int dim);
    /* helper function to find median with quickselect algorithm */
    void select(vector<Point<Dim>>& points, int leftIdx, int rightIdx, int expectedMedianIdx, int dim);
    /* helper function to partition the list, left(smaller than medianIdx) and right(larger than medianIdx)*/
    int partition(vector<Point<Dim>>& points,int leftIdx, int rightIdx, int pivotIndex, int dim);
    /* helper function to swap points */
    void swapPoints(vector<Point<Dim>>& points, int a, int b);
    /* helper function to copy the tree from root */
    KDTree<Dim>::KDTreeNode* copyTree(KDTreeNode* subroot);
    /* helper function to delete the tree*/
    void deleteTree(KDTreeNode* subroot);
    /* helper function to recursively findNearestNeighbor()*/
    void findNearestNeighbor(KDTreeNode* subroot, const Point<Dim>& query, Point<Dim>& currentBest, int dimension) const;
};
