                       ...
                       map<Point<3>,string>& theTiles)
{
    ...
    for(int row=0;row<theSource.getRows();row++){
        for(int column=0;column<theSource.getColumns();column++){
            newMosaicCanvas->setTile(row,column,get_match_at_idx(tree,theTiles,theSource,row,column));
        }
    }
    return newMosaicCanvas;
}

string* get_match_at_idx(const KDTree<3>& tree,
                                  map<Point<3>,string>& theTiles,
                                  ...
                                  int col)
{
    ...
    auto it = theTiles.find(nearestPoint);// iterator->auto
