...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if(subroot->elements.empty()) return V();
    size_t first_larger_idx = insertion_idx(subroot->elements, key);
    if(first_larger_idx>subroot->elements.size()) return V();
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    auto right_elem_itr=child->elements.begin() + mid_elem_idx+1;
    ...
    new_right->elements.assign(right_elem_itr,child->elements.end());
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if(subroot->is_leaf)
        ...
        insert(child,pair);
        if(child->elements.size()>order-1) split_child(subroot,first_larger_idx);
    }
}
