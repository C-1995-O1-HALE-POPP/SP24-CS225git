...
{
    // xzh: can not insert an existed node
    ...
    while (traverse->nodePointers[0].next != tail && (level >= 0))
    {
        ...
        else traverse = traverse->nodePointers[level].next;

    }

    // expand head and tail
    ...
    // create containers and link level by level
    ...
    {
        if(forward == tail||forward->nodePointers.size() > (size_t)forwardLevel)
        {
            forward->nodePointers[forwardLevel].prev = newNode;
            newNode->nodePointers[forwardLevel].next = forward;
            forwardLevel++;
        }
        else forward = forward->nodePointers[0].next;
    }

    while (backwardLevel < maxLevel)
    {
        if(prev == head||prev->nodePointers.size() > (size_t)backwardLevel)
        {
...
{
    SkipNode * retval;
    HSLAPixel ret = HSLAPixel(0,0,0, 50);
