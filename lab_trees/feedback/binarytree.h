...
class BinaryTree
{
  ...
  private:
    ...
    bool isOrderedHelper(const Node* node,const Node* last) const;

    void printPathsHelper(Node* node, vector<T> curr,vector<vector<T> > &paths) const;

    void sumDistancesHelper(Node* node, int& sum, int depth) const;
    /**
