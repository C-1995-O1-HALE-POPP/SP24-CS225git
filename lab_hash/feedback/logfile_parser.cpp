...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
        ...
        string request=ll.customer+ll.url;
        if((!whenVisitedTable.keyExists(request))||ll.date>whenVisitedTable[request])
            whenVisitedTable[request]=ll.date;
    }
    infile.close();
}
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
    ...
    string request=customer+url;
    return whenVisitedTable.keyExists(request);
                                  ...
                                  const string& url) const
{
    ...
    //(void) url;      // prevent warnings... When you implement this function, remove this line.
    string request=customer+url;
    return whenVisitedTable.find(request); // replaceme
}
