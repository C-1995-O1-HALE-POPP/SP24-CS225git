...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
    ...
    Dict<char,int> wordTable(128);
    Dict<char,int> testTable(128);
    for (size_t i = 0; i < word.length(); i++) {
        wordTable[word[i]]++;
        testTable[test[i]]++;
    }
    typename Dict<char,int>::iterator itWord = wordTable.begin();
    typename Dict<char,int>::iterator itTable = testTable.begin();
    while(itWord!=wordTable.end()){
        if(itWord->second!=itTable->second){
            return false;
        }
        itWord++;
        itTable++;
